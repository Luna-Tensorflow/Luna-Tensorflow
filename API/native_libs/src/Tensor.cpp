...
Tensor<DataTypeLabel>::Tensor(const std::vector<Tensor::type> &vect)
{
	size_t data_size = TF_DataTypeSize(DataTypeLabel);
	dims = std::vector<size_t>{vect.size()};
	underlying = TF_AllocateTensor(DataTypeLabel, dims.data(), 1, data_size * vect.size());
