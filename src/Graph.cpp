...
TF_Operation* Graph::make_operation(const std::string &type, const std::string &name, const std::vector<TF_Output> &inputs, const std::vector<std::shared_ptr<Attr>> &attrs) {
...
TF_Output Graph::get_output(TF_Operation *operation, int index) {
    return {
            .oper = operation,
            .index = index
    };
...
TF_Operation* Graph::make_variable(const std::string &name, const std::vector<int64_t> &dims, TF_DataType dtype) {
    return make_operation("Variable", name, {},
                          {std::make_shared<AttrShape>("shape", dims), std::make_shared<AttrType>("dtype", dtype)});
}

TF_Operation* Graph::make_constant(const std::string &name, const Tensor &tensor) {
...
TF_Operation* Graph::make_assign(const std::string &name, TF_Output variable, TF_Output value) {
    return make_operation("Assign", name,
                          {variable, value}, {});
}

TF_Operation* Graph::make_variable_init(std::string name, TF_Output variable, const Tensor &tensor) {
    TF_Output value = get_output(make_constant(name + "_val", tensor), 0);
    return make_assign(name + "_assign", variable, value);
}

TF_Operation* Graph::make_assign_sub(const std::string &name, TF_Output variable, TF_Output value) {
    return make_operation("AssignSub", name,
                          {variable, value}, {});
}

TF_Operation* Graph::make_placeholder(const std::string &name, const std::vector<int64_t> &dims, TF_DataType dtype) {
    return make_operation("Placeholder", name, {},
                          {std::make_shared<AttrShape>("shape", dims), std::make_shared<AttrType>("dtype", dtype)});
}

TF_Operation* Graph::make_addition(const std::string &name, TF_Output a, TF_Output b) {
    return make_operation("Add", name,
                          {a, b}, {});
}

TF_Operation* Graph::make_substraction(const std::string &name, TF_Output a, TF_Output b) {
    return make_operation("Sub", name,
                          {a, b}, {});
}

TF_Operation* Graph::make_mul(const std::string &name, TF_Output a, TF_Output b) {
    return make_operation("Mul", name,
                          {a, b}, {});
}

TF_Operation* Graph::make_square(const std::string &name, TF_Output a) {
    return make_operation("Square", name,
                          {a}, {});
}

std::vector<TF_Output> Graph::make_gradient(std::vector<TF_Output> ys, std::vector<TF_Output> xs) {
...
std::vector<Tensor> Graph::run_session(
